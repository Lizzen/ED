...
public:
	...
	/** INDICA AQUÍ JUSTIFICADAMENTE LA COMPLEJIDAD DE LA OPERACIÓN minimoACima
		La complejidad del código es O(n), donde n es el número de elementos de la pila ya que los dos recorridos de la pila son lineales.
	**/
	void minimoACima() {
		// IMPLEMENTA AQUÍ LA OPERACIÓN
		if (!esVacia()) {
			int minimo = _cima->_elem;
			Nodo* cima = _cima;
			Nodo* min = nullptr;
			Nodo* ant = nullptr;
			Nodo* elemento = _cima;
			
			while (elemento->_sig != nullptr) {
				if (minimo > elemento->_sig->_elem) {
					minimo = elemento->_sig->_elem;
					min = elemento->_sig;
					ant = elemento;
				}
				elemento = elemento->_sig;
			}

			if (minimo != _cima->_elem) {
				ant->_sig = min->_sig;
				min->_sig = cima;
				_cima = min;
			}
