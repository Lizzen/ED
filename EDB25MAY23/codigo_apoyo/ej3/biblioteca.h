...
class infoSocios {
public:
    infoSocios(tNombre nombre, Diccionario<tSignatura, tFecha> sig_fecha) :
        nombre(nombre), sig_fecha(sig_fecha) {};
    tNombre nombre;
    Diccionario<tSignatura, tFecha> sig_fecha;
};


class infoLibros {
public:
    infoLibros(tNumEjemplares num_ejemplares, Lista<tCodigo> espera) :
        num_ejemplares(num_ejemplares), espera(espera) {};
    tNumEjemplares num_ejemplares;
    Lista<tCodigo> espera = Lista<tCodigo>();
};
...
private:  
    Diccionario<tSignatura, infoLibros> libros = Diccionario<tSignatura, infoLibros>();
    Diccionario<tCodigo, infoSocios> socios = Diccionario<tCodigo, infoSocios>();
};
