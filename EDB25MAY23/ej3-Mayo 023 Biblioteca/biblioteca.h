...
private:    
    struct infoSocio {
        tNombre nombre;
        Diccionario<tSignatura, tFecha> SyF;
    };

    struct infoLibro {
        tNumEjemplares n;
        Lista<Codigo_y_Nombre> espera;
    };

    DiccionarioHash<tSignatura, infoLibro> libros = DiccionarioHash<tSignatura, infoLibro>();
    DiccionarioHash<tCodigo, infoSocio> socios = DiccionarioHash<tCodigo, infoSocio>();
};
