/* Indica aquí nombre y apellidos del/de los miembro(s) del grupo
NOMBRE Y APELLIDOS MIEMBRO 1: David Ferreras Díaz
NOMBRE Y APELLIDOS MIEMBRO 2: David Díaz Gregorio
GRUPO: B17
*/
...
public:
	...
	/*
	 ...
	 La complejidad del algoritmo es O(n) ya que se recorre todos los nodos de la lista una vez.
	*/
	void estremece() {
		if (_numElems > 2) {

			Nodo* par = _prim, * ultimo = _ult, * impar = nullptr;
			Nodo* a = _prim->_sig;
			Nodo* b = nullptr;
			...
			while (a != _ult) {
				if (i % 2 == 0) {
					b = a;
					a = a->_sig;
					// Mueve el primer impar al último elemento de la lista, por lo tanto el último par apuntará a él
					if (impar == nullptr) {
						impar = b;
						impar->_sig = nullptr;
						_ult = impar;
					...
					// Mueve el elemento impar delante del ultimo elemento impar movido
					else {
						impar->_ant = b;
						b->_sig = impar;
						impar = impar->_ant;
				...
				// Mueve el elemento par detras del último par movido
				else {
					b = a;
					a = a->_sig;
					par->_sig = b;
