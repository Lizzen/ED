...
public:
	...
	*/
	...
	Lista extrae_e_inserta(unsigned int pos_inicio, unsigned int n_elems_a_extraer,
		                   Lista& elems_a_insertar) {
		// A IMPLEMENTAR

		Lista<T> ret;

		if (!elems_a_insertar.esVacia() && n_elems_a_extraer > 0 && pos_inicio < _numElems - 1 && !esVacia()) {
			Nodo* act = _prim, *elem = nullptr, *fin_elem = nullptr;
			
			int pos_final = pos_inicio + n_elems_a_extraer - 1;

			if (pos_final > _numElems - 1) {
				pos_final = _numElems - 1;
			}
			...
			if (elem != _prim && fin_elem != _ult) {
				elems_a_insertar._prim->_ant = elem->_ant;
				elem->_ant->_sig = elems_a_insertar._prim;
				elems_a_insertar._ult->_sig = fin_elem->_sig;
				fin_elem->_sig->_ant = elems_a_insertar._ult;
				elem->_ant = nullptr;
				fin_elem->_sig = nullptr;
			}
			else if (elem == _prim && fin_elem != _ult) {
				elems_a_insertar._ult->_sig = fin_elem->_sig;
				fin_elem->_sig->_ant = elems_a_insertar._ult;
				_prim = elems_a_insertar._prim;
				fin_elem->_sig = nullptr;
			}
			else if (elem != _prim && fin_elem == _ult) {
				elems_a_insertar._prim->_ant = elem->_ant;
				elem->_ant->_sig = elems_a_insertar._prim;
				_ult = elems_a_insertar._ult;
				elem->_ant = nullptr;
			}
			else {
				_prim = elems_a_insertar._prim;
			...
			ret._prim = elem;
			ret._ult = fin_elem;
			elems_a_insertar._prim = nullptr;
			elems_a_insertar._ult = nullptr;
		}
		return ret;
	}
...
public:
	...
	void imprime_invertida(ostream& cout) {
		Nodo* act = _ult;
		while (act != _prim) {
			cout << act->_elem << " ";
			act = act->_ant;
